...
export const user = createSlice({
  ...
  reducers: {
    ...
    signIn: (state, action) => {
      ...
      const stateCurrentUser = state.userList.find(
    ...
    changeCart: (state, action) => {
      ...
      if (parseCartState[0]) {
        const sameUser = state.userList.find(
          (user) => user.id == parseCartState[0].id
        );
    ...
    buyCheckedProduct: (state, action) => {
      ...
      const sameUser = state.userList.find(
        (user) => user.id == purchaseArray[0].id
      );
      console.log(sameUser);
    ...
    limitReview: (state, action) => {
      ...
      const sameUser = state.userList.find(
        (user) => user.id == reviewInfo.currentUser
      );
      console.log(sameUser);
      const sameReview = sameUser.orderedProducts.find(
        (r) => r.myPageId == reviewInfo.myPageId
      );
